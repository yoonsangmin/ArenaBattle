...
void AABCharacterPlayer::SetDead()
{
	...
	// 5초 후에 앞서 비활성화했던 항목 복구.
	GetWorld()->GetTimerManager().SetTimer(DeadTimerHandle, this, &AABCharacterPlayer::ResetPlayer, RespawnTime, false);
...
void AABCharacterPlayer::OnRep_Owner()
{
	...
	// Super::OnRep_Owner() 호출 전 액터의 소유 확인.
...
void AABCharacterPlayer::Attack()
{
	...
	{
		// 클라이언트 로직.
		// 클라이언트 입장에서는 Local:AutonomousProxy/Remote:Authority.
		if (!HasAuthority())
		{
			// 공격 다시 못하게 설정.
			bCanAttack = false;

			// bCanAttack 값이 변경되지 않아, 리플리케이션이 되지 않기 때문에
			// OnRep_ 함수 호출을 기대하지 않고, 직접 모드 변경.
...
void AABCharacterPlayer::UpdateMeshFromPlayerState()
{
	...
	int32 MeshIndex = FMath::Clamp(GetPlayerState()->GetPlayerId() % PlayerMeshes.Num(), 0, GetPlayerState()->GetPlayerId() % PlayerMeshes.Num() - 1);
...
void AABCharacterPlayer::DrawDebugAttackRange(const FColor& DrawColor, FVector TraceStart,
	FVector TraceEnd, FVector Forward)
{
...
void AABCharacterPlayer::ServerRPCAttack_Implementation(float AttackStartTime)
{
	...
	// 시간 차이 값을 로그로 확인.
	AB_LOG(LogABNetwork, Log, TEXT("Lag Time: %f"), AttackTimeDifference);

	// 예외 처리.
	// 네트워크 상태가 너무 안 좋은 경우, AttackTimeDifference 값이 너무 크게되면,
	// 아래의 타이머가 작동하지 않을 수 있기 때문에 최소한의 시간 값을 보정.
...
{
	// 서버와 본인(입력을 전달한) 클라이언트는 이미
	// 공격 상태 값과 무브먼트 모드 값을 변경함.
	// 여기에서는 나머지 클라이언트에 대한 처리를 진행.
	// 공격 상태 값과 무브먼트 모드 값은 프로퍼티 리플리케이션으로 전달되어 처리가 될 것.
...
{
	// 클라이언트로부터 공격 관련 시간 값이 전달 안 됐으면, 검증 안 함.
	if (LastAttackStartTime == 0.0f)
	{
		return true;
	}

	// 공격을 시작한 후 공격을 판정한 시간 사이에 걸린 시간이
	// 공격 판정 최소 기준을 넘어가는지 확인.
	return (HitCheckTime - LastAttackStartTime) > AceeptMinCheckTime;
}
...
void AABCharacterPlayer::ServerRPCNotifyHit_Implementation(const FHitResult& HitResult, float HitCheckTime)
{
	...
	if (IsValid(HitActor))
	{
		...
		DrawDebugPoint(GetWorld(), ActorBoxCenter, 30.0f, FColor::Magenta, false, 5.0f);
#endif
